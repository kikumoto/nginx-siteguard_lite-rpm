...
build:
	[ -d $@.bak ] && rm -rf $@.bak || :
	[ -d $@ ] && mv $@ $@.bak || :
	docker build -f Dockerfile -t ${IMAGE_NAME} --build-arg nginx_src_rpm=${NGINX_SRC_RPM} --build-arg siteguard_url=${SITEGUARD_URL} --build-arg siteguard_ver=${SITEGUARD_VER} .
